...
export const Module4NpsAdoption: React.FC<Module4Props> = ({ kpis, closedProjects, theme = 'dark' }) => {
  ...
  const npsBreakdown = [
    { type: 'Promotores (Score 9-10)', pct: 65, count: 156, color: '#10b981' },
    { type: 'Pasivos (Score 7-8)', pct: 25, count: 60, color: '#f59e0b' },
    { type: 'Detractores (Score <7)', pct: 10, count: 24, color: '#ef4444' },
  ];

  const departmentAdoption = [
    { dept: 'Ventas (CRM 360°)', activeUsers: 450, totalUsers: 490, adoptionPct: 92, status: 'Alta' },
    { dept: 'Operaciones (IA Contact Center)', activeUsers: 380, totalUsers: 420, adoptionPct: 90, status: 'Alta' },
    { dept: 'Finanzas (Automatización SAP)', activeUsers: 176, totalUsers: 200, adoptionPct: 88, status: 'Alta' },
    { dept: 'Atención Clientes (Portal Cliente)', activeUsers: 180, totalUsers: 300, adoptionPct: 60, status: 'Media' },
    { dept: 'Marketing (App Móvil)', activeUsers: 110, totalUsers: 200, adoptionPct: 55, status: 'En Enfoque' },
  ];

  const userFeedbackList = [
    { id: 1, project: 'IA Contact Center', user: 'Lic. Sofía R.', area: 'Operaciones', comment: 'El asistente con IA redujo el tiempo de atención en llamadas un 40%. La interfaz es muy intuitiva.', nps: 10, type: 'positive' },
    { id: 2, project: 'CRM 360°', user: 'Ing. Mateo G.', area: 'Ventas', comment: 'Excelente visibilidad de prospectos. Sugiero integrar un reporte semanal automatizado.', nps: 9, type: 'positive' },
    { id: 3, project: 'Automatización SAP', user: 'C.P. Elena V.', area: 'Finanzas', comment: 'El proceso de conciliación bancaria pasó de 3 días a 2 horas. Gran impacto operativo.', nps: 9, type: 'positive' },
    { id: 4, project: 'Portal del Cliente', user: 'Lic. Javier M.', area: 'Servicios', comment: 'Se requiere una sesión adicional de capacitación para usuarios del turno nocturno.', nps: 7, type: 'neutral' },
  ];
    ...
    <div className="space-y-4 text-left">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
        ...
        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${
          ...
          <div className="mt-1 flex items-baseline justify-between">
            <span className="text-xl font-black text-emerald-500">65%</span>
        ...
        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${
          ...
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Capacitación</span>
          <div className="mt-1 flex items-baseline justify-between">
            <span className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>480 hrs</span>
          </div>
        </div>
        ...
        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${
          ...
          <div className="mt-1 flex items-baseline justify-between">
            <span className="text-xl font-black text-cyan-500">92%</span>
        ...
        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${
          ...
          <div className="mt-1 flex items-baseline justify-between">
            <span className="text-xl font-black text-emerald-500">3 <span className="text-[9px] font-normal text-slate-400">proyectos</span></span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-3">
        ...
        <div className={`xl:col-span-6 p-3.5 rounded-xl border transition-colors ${
          ...
          <h3 className={`text-xs font-bold tracking-wide uppercase mb-2 pb-1.5 border-b ${
            ...
            DESGLOSE DE SATISFACCIÓN NPS (240 RESPUESTAS)
          </h3>
          ...
          <div className={`mt-3 p-2.5 rounded-lg border text-xs flex items-center justify-between ${
            ...
            <strong className="text-emerald-500 text-sm font-extrabold">+55 NPS Net</strong>
          </div>
        </div>
        ...
        <div className={`xl:col-span-6 p-3.5 rounded-xl border transition-colors ${
          ...
          <h3 className={`text-xs font-bold tracking-wide uppercase mb-2 pb-1.5 border-b ${
            ...
            ADOPCIÓN Y USUARIOS ACTIVOS POR DEPARTAMENTO
          </h3>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className={`border-b text-[9px] uppercase ${isDark ? 'border-[#1d2d4f] text-slate-400' : 'border-slate-200 text-slate-500'}`}>
                  ...
                  <th className="py-1 px-1.5 text-center">Usuarios Activos</th>
              ...
              <tbody className={`divide-y ${isDark ? 'divide-[#152342]' : 'divide-slate-100'}`}>
                {departmentAdoption.map((row) => (
                  <tr key={row.dept} className={`transition-colors ${isDark ? 'hover:bg-[#132244]' : 'hover:bg-slate-50'}`}>
