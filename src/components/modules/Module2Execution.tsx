    ...
    <div className="space-y-4 text-left">
      ...
      <div className={`border rounded-xl p-3 flex items-center justify-between shadow-md transition-colors ${
        ...
        <div className="flex items-center space-x-2.5">
          <div className="w-7 h-7 rounded-lg bg-cyan-600 flex items-center justify-center font-extrabold text-white text-sm shadow">
            2
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Proyectos Activos</span>
          <div className="mt-1 flex items-baseline justify-between">
            <span className={`text-xl font-extrabold ${isDark ? 'text-white' : 'text-slate-900'}`}>{kpis.activeProjects}</span>
            <Kanban className="w-3.5 h-3.5 text-cyan-500" />
          </div>
        </div>

        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>On Track</span>
          <div className="mt-1 flex items-baseline justify-between">
            <span className="text-xl font-extrabold text-emerald-500">{kpis.pctOnTrack}%</span>
            <CheckCircle className="w-3.5 h-3.5 text-emerald-500" />
          </div>
        </div>

        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>En Riesgo</span>
          <div className="mt-1 flex items-baseline justify-between">
            <span className="text-xl font-extrabold text-amber-500">{kpis.projectsInRisk}</span>
            <AlertTriangle className="w-3.5 h-3.5 text-amber-500" />
          </div>
        </div>

        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Atrasados</span>
          <div className="mt-1 flex items-baseline justify-between">
            <span className="text-xl font-extrabold text-rose-500">{kpis.projectsDelayed}</span>
            <XCircle className="w-3.5 h-3.5 text-rose-500" />
          </div>
        </div>

        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Avance Promedio</span>
          <div className="mt-1 flex items-baseline">
            <span className="text-xl font-extrabold text-blue-500">{kpis.avgProgressPct}%</span>
          </div>
        </div>

        <div className={`p-2.5 rounded-xl border flex flex-col justify-between transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          <span className={`text-[10px] font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Presupuesto Consumido</span>
          <div className="mt-1">
            <span className="text-lg font-extrabold text-emerald-500">${kpis.consumedBudgetMXN}M</span>
            <span className={`text-[9px] ml-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>de ${kpis.totalApprovedBudgetMXN}M</span>
          </div>
        </div>
      </div>
      ...
      <div className={`p-3.5 rounded-xl border transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className={`flex items-center justify-between mb-2 pb-1.5 border-b ${isDark ? 'border-[#1d2d4f]' : 'border-slate-200'}`}>
          ...
          <div className={`flex items-center space-x-3 text-[9px] ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-amber-500 inline-block"></span> En Riesgo</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-rose-500 inline-block"></span> Atrasado</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-slate-500 inline-block"></span> Sin Iniciar</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-3">
        ...
        <div className={`xl:col-span-6 p-3.5 rounded-xl border transition-colors ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
          ...
          <div className="grid grid-cols-2 gap-3 items-center">
            <div className={`border rounded-xl p-2.5 text-center flex flex-col items-center ${isDark ? 'bg-[#091224] border-[#1a2b4e]' : 'bg-slate-50 border-slate-200'}`}>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className={`border-b text-[9px] uppercase ${isDark ? 'border-[#1d2d4f] text-slate-400' : 'border-slate-200 text-slate-500'}`}>
                  ...
                  <th className="py-1 px-1.5 text-center">Tiempo</th>
                  <th className="py-1 px-1.5 text-center">Coste</th>
                  <th className="py-1 px-1.5 text-center">Alcance</th>
                  <th className="py-1 px-1.5 text-center">Riesgos</th>
                </tr>
              </thead>
              <tbody className={`divide-y ${isDark ? 'divide-[#152342]' : 'divide-slate-100'}`}>
