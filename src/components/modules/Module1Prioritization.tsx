...
export const Module1Prioritization: React.FC<Module1Props> = ({ kpis, initiatives, theme = 'dark', onDrillDown }) => {
  ...
  return (
    <div className="space-y-4 text-left w-full select-none max-w-full overflow-hidden">
      ...
      {/* Main Content: Matriz Valor vs Esfuerzo & Top 10 Ranking */}
      <div className="space-y-4">
        {/* Matriz Valor vs Esfuerzo (1:1 Isometric Proportional Square Matrix) */}
        <div className={`p-3.5 sm:p-5 rounded-xl border flex flex-col justify-between transition-colors min-w-0 ${
          isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'
        }`}>
          <h3 className={`text-center text-xs sm:text-sm font-extrabold tracking-wide uppercase mb-2 sm:mb-3 ${isDark ? 'text-white' : 'text-[#0f172a]'}`}>
            MATRIZ VALOR VS ESFUERZO
          </h3>

          <div className="relative flex items-center justify-center p-1 sm:p-2 my-auto w-full">
            {/* Y Axis Label & Arrow */}
            ...
            {/* Matrix Proportional Square Canvas */}
            <div className="ml-7 sm:ml-8 mr-1 sm:mr-2 w-full max-w-[320px] aspect-square border-2 rounded-xl relative overflow-hidden transition-colors border-slate-300 dark:border-slate-700 bg-white dark:bg-[#070e1c] shadow-inner">
              ...
              {/* Quadrant Titles - Anchored safely to Left & Right margins */}
              <div className="absolute top-2.5 left-2.5 z-0 pointer-events-none">
                <span className="font-black text-[9px] sm:text-xs text-[#16a34a] uppercase tracking-wide whitespace-nowrap">
                  Quick Wins
                </span>
              </div>

              <div className="absolute top-2.5 right-2.5 z-0 pointer-events-none text-right">
                <span className="font-black text-[9px] sm:text-xs text-[#2563eb] uppercase tracking-wide whitespace-nowrap">
                  Apuestas Estratégicas
                </span>
              </div>

              <div className="absolute bottom-2.5 left-2.5 z-0 pointer-events-none">
                <span className="font-black text-[9px] sm:text-xs text-[#ca8a04] uppercase tracking-wide whitespace-nowrap">
                  Relleno
                </span>
              </div>

              <div className="absolute bottom-2.5 right-2.5 z-0 pointer-events-none text-right">
                <span className="font-black text-[9px] sm:text-xs text-[#dc2626] uppercase tracking-wide whitespace-nowrap">
                  Baja Prioridad
                </span>
              </div>
              ...
              {scatterDots.map((dot, idx) => {
                ...
                return (
                  ...
                  >
                    <div className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-36 sm:w-40 p-2 rounded-lg bg-slate-900 text-white text-[10px] shadow-2xl z-50 pointer-events-none border border-slate-700 text-center">
          ...
          {/* X Axis Label & Arrow */}
          <div className="ml-7 sm:ml-8 flex items-center justify-between text-[10px] sm:text-xs font-extrabold text-slate-700 dark:text-slate-300 pt-1 sm:pt-2">
            <span>Bajo</span>
            <div className="flex items-center space-x-1.5 font-black uppercase tracking-wider">
              <span>ESFUERZO</span>
              <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-700 dark:text-slate-300" />
            </div>
            <span>Alto</span>
          </div>
        </div>
        ...
        <div className={`p-3.5 sm:p-4 rounded-xl border flex flex-col justify-between transition-colors min-w-0 ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
            ...
            <table className="w-full text-left text-xs min-w-[260px]">
              <thead>
                <tr className={`border-b text-[10px] uppercase ${isDark ? 'border-[#1d2d4f] text-slate-400' : 'border-slate-200 text-slate-500'}`}>
                  ...
                  <th className="py-1.5 px-1.5 text-right">ROI</th>
              ...
              <tbody className={`divide-y ${isDark ? 'divide-[#152342]' : 'divide-slate-100'}`}>
                {filteredInitiatives.slice(0, 10).map((item) => (
                  ...
                  >
                    ...
                    <td className={`py-1.5 px-1.5 font-bold text-xs truncate max-w-[120px] ${isDark ? 'text-slate-100' : 'text-slate-800'}`}>{item.name}</td>
                    <td className="py-1.5 px-1.5 text-center">
                      <span className={`px-2 py-0.5 rounded text-[10px] sm:text-[11px] font-black ${
                    ...
                    <td className="py-1.5 px-1.5 text-center text-cyan-400 hover:text-white">
                      <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4 inline" />
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className={`mt-2 text-[9px] sm:text-[10px] italic text-right ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
      ...
      <div className="space-y-3">
        <div className={`p-3.5 sm:p-4 rounded-xl border transition-colors min-w-0 ${isDark ? 'bg-[#0e172a] border-[#1e293b]' : 'bg-white border-slate-200 shadow-sm'}`}>
              ...
              {categories.map((cat) => (
                <div key={cat.name} className="flex items-center justify-between text-xs">
                  <div className="flex items-center space-x-2 min-w-0">
                    ...
                    <span className={`truncate ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{cat.name}</span>
                  </div>
                  <span className={`font-black ml-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{cat.pct}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={`border rounded-xl p-3.5 sm:p-4 flex items-center justify-between transition-colors min-w-0 ${
          isDark ? 'bg-[#091224] border-[#1b2c4e]' : 'bg-blue-50/70 border-blue-200'
        }`}>
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-full border flex-shrink-0 ${isDark ? 'bg-blue-900/50 text-cyan-300 border-blue-500/30' : 'bg-blue-100 text-blue-700 border-blue-300'}`}>
            ...
            <div>
              ...
              <p className={`text-[10px] sm:text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Mayor score = Mayor prioridad. Forms 1.
              </p>
            </div>
          </div>
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg whitespace-nowrap hidden sm:inline-block border ${
